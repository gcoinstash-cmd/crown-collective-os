...
export default function AdminPortalModal({ isOpen, onClose }: AdminPortalModalProps) {
  ...
  const handleVerify = (e: React.FormEvent) => {
    e.preventDefault();
    if (passkey.trim().toLowerCase() === 'crown2026') {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
      <div className="relative w-full max-w-lg bg-[#0E0F12] border border-[#2A2B30] rounded-2xl p-6 sm:p-8 shadow-2xl text-stone-100">
        <button
          onClick={onClose}
        ...
        {!isAuthenticated ? (
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-light tracking-tight text-white font-serif">Crown & Collective Atelier OS</h3>
                <p className="text-xs text-stone-400 font-mono">Melrose Master Stylist & Floor Governance</p>
              </div>
            </div>
            ...
            <form onSubmit={handleVerify} className="space-y-4">
              <div>
                ...
                {error && (
                  <p className="text-xs text-red-400 mt-2 font-mono">Invalid atelier passkey. Try: crown2026</p>
                )}
              </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-emerald-400 border-b border-stone-800 pb-4">
              ...
              <div>
                <h4 className="text-base font-medium text-white">Master Colorist & Stylist Authenticated</h4>
                <p className="text-xs text-stone-400 font-mono">Passkey: crown2026 • Clearance Level: Atelier Director</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-left font-mono text-xs">
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Chairs Booked</span>
                <span className="text-stone-100 font-semibold text-sm">6 Haute Suites</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Active Bookings</span>
                <span className="text-amber-400 font-semibold text-sm">14 Confirmed</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Weekly Volume</span>
                <span className="text-emerald-400 font-semibold text-sm">$18,450.00</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
